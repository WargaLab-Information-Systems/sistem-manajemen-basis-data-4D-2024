...
-- 1
CREATE VIEW total_pemesanan AS SELECT a.nama_pembeli, b.total_harga, b.tanggal_transaksi
FROM transaksi b JOIN pembeli a ON b.id_pembeli = a.id_pembeli
WHERE b.total_harga > (SELECT AVG(total_harga) FROM transaksi);
-- 2
CREATE VIEW stok AS SELECT nama_barang, stok_barang FROM barang WHERE stok_barang <10;
-- 3
CREATE VIEW detai_tarnsaksi AS 
	SELECT transaksi_id, tanggal_transaksi, jumlah_barang, total_harga, penjual_id, pembeli_id FROM transaksi;
	
SELECT * FROM stok
SELECT * FROM detai_tarnsaksi

-- 4
DELIMITER //
CREATE PROCEDURE GetDatabarang(IN nama_databarang VARCHAR(100),IN stok_databarang INT)
BEGIN
 SELECT nama_databarang, stok_databarang FROM barang WHERE nama_databarang LIKE CONCAT ('%', nama_databarang, '%') 
 AND stok_databarang > 6 ;
END//
DELIMITER ;
CALL GetDatabarang ('e',)
DROP PROCEDURE GetDatabarang

SELECT nama_barang, stok_barang FROM barang WHERE nama_barang LIKE '%e%' AND stok_barang >6;

-- 3
DELIMITER //
CREATE PROCEDURE Jumlahstokbarang(OUT stok_barang INT)
BEGIN
 SELECT SUM(stok_barang) INTO stok_barang FROM barang;
END //
DELIMITER ;
CALL Jumlahstokbarang(@jumlah);
SELECT @jumlah

DROP PROCEDURE GetJumlahbarang

-- 4
DELIMITER //
CREATE PROCEDURE input_supplier(IN supplier_id INT ,
...
END //
DELIMITER ;
CALL input_supplier (11, 'PT HAHA', 'Jl. Timur Laut', '081234568901');
SELECT * FROM supplier

-- 5
DELIMITER //
CREATE PROCEDURE update_pembeli( IN nama_pembeli VARCHAR(100))
BEGIN
	UPDATE TABLE pembeli SET nama_pembeli WHERE 
END //
DELIMITER ;