...
SELECT nama_pembeli, total_harga FROM barang 
where t(avg > total_harga)  ;


-- soal 2 view
CREATE VIEW stok_kurang_10 AS
SELECT nama_barang, stok_barang FROM barang 
where stok_barang < 10  ;
 select * from  stok_kurang_10;
...
-- 1 stored procedure
DELIMITER //
CREATE PROCEDURE datanama(
    IN barang_id INT,
    IN nama_barang VARCHAR(100),
    IN harga_barang DOUBLE,
    IN stok_barang INT,
    IN supplier_id INT,
    IN penjual_id INT
    )
...
END //
DELIMITER ;
CALL input_supplier(11, 'SEMESTA', 'Jl. jakarta ', '08123897867');
...
BEGIN 
   update pembeli supplier_id, nama_supplier, alamat_supplier, telepon_supplier ;
END //
DELIMITER ;
CALL update_pembeli(11, 'SEMESTA', 'Jl. jakarta ', '08123897867');
 SELECT * FROM pembeli;

-- 6 stored
DELIMITER //
CREATE PROCEDURE hapus_tbl() 
BEGIN 
    DROP table gudang ;
END //
DELIMITER ;
