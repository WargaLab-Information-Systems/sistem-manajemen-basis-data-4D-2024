...
(6, 'CV Jaya Baru', 'Jl. Gatot Subroto No. 101', '08167890123', 'info@cvjayabaru.com');
...
(6, 'Gudang Sejahtera', 'Jl. Sudirman No. 50');
...
(6, 6, 'Nina', 'Jl. Pahlawan No. 55', 'Staf', 6000000);
...
(6, 6, 6, 3, 30, 750000, '2024-05-10 10:30:00');
...
BEGIN
    SELECT 
        SUM(total_harga) 
    INTO 
        total_harga_baru --  untuk menyimpan hasil query
    FROM 
        transaksi
    WHERE 
...
END //
DELIMITER ;
CALL HitungTotalTransaksiKaryawan(3, @total_harga);
...
BEGIN
    SELECT 
        ...
        DATEDIFF(CURDATE(), s.tanggal_update) AS lama_hari -- menghitung selisih hari dari tgl saat ini
    FROM 
        stok s
    INNER JOIN 
...
BEGIN
    -- Membuat variabel untuk tanggal 1 bulan yang lalu
    DECLARE tanggal_awal DATE;
    SET tanggal_awal = DATE_SUB(CURDATE(), INTERVAL 1 MONTH);

    -- Menghapus transaksi pada 1 bulan terakhir dengan total harga kurang dari atau sama dengan 200 ribu
    DELETE FROM transaksi
    WHERE tanggal_transaksi >= tanggal_awal
    AND total_harga <= 200000
    AND id_transaksi > 0; -- Tambahkan kondisi WHERE dengan kolom kunci

    SELECT 'berhasil dihapus.';
END //
DELIMITER ;
