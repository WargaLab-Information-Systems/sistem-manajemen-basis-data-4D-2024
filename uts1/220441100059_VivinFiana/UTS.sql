...
CREATE PROCEDURE data_barang(
	IN barang VARCHAR(100),
    IN stok INT
)
BEGIN
	SELECT * FROM barang where barang = nama_barang like '%e%' AND stok = stok_barang > 6;
END //
DELIMITER ;
CALL data_barang();
DROP PROCEDURE data_barang;

--NO 2
DELIMITER //
CREATE PROCEDURE data_transaksi(
	IN tgl_transsaksi DATE,
    IN jlh_barang INT(11)
)
BEGIN
	SELECT * FROM transaksi where tgl_transsaksi = tanggal_transaksi
END //
DELIMITER ;
call data_transaksi
...
--NO 4
DELIMITER //
CREATE PROCEDURE input_supllier(
	IN supplier_id INT,
    IN nama_suppier VARCHAR(100),
    IN alamat_supplier VARCHAR(255),
    IN telelepon_supplier VARCHAR(20)
)
BEGIN 
	INSERT INTO input_supplier values (supplier_id, nama_suppier, alamat_supplier, telelepon_supplier);
END //
DELIMITER ;
call input_suppier(11, 'Nino', 'Telang', '12345678');
...
CREATE PROCEDURE update_pembeli(
    ...
    IN telelepon_pembeli VARCHAR(20)
)
BEGIN 
	INSERT INTO update_pembeli values (pembeli_id, nama_pembeli, alamat_pembeli, telelepon_pembeli);
END //
DELIMITER ;
call input_suppier(11, 'Jeno', 'Kamal', '123456789');