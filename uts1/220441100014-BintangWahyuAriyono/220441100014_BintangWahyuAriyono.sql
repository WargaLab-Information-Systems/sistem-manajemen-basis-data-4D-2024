...
-- view
-- 1
CREATE VIEW diatas_rata AS 
SELECT p.nama_pembeli, t.total_harga, t.tanggal_transaksi
FROM transaksi AS t JOIN pembeli AS p ON t.pembeli_id = p.pembeli_id
WHERE t.total_harga > (
  SELECT AVG(total_harga)
  FROM transaksi
);
SELECT * FROM diatas_rata;

-- 2
CREATE VIEW stok AS
SELECT s.nama_barang, s.stok_barang FROM barang AS s
WHERE s.stok_barang < 10;
SELECT * FROM stok;

-- 3
CREATE VIEW detail_trans AS
...
-- 2
DELIMITER //
CREATE PROCEDURE tampil_data()
BEGIN
  SELECT * FROM transaksi WHERE MONTH(tanggal_transaksi) = 4
    AND DAY(tanggal_transaksi) BETWEEN 3 AND 9 AND jumlah_barang > 1;
END //
DELIMITER ;
CALL tampil_data;
...
BEGIN 
	SELECT * FROM barang WHERE (sum(barang.stok_barang)
END//
DELIMITER ;
CALL jumlah_total;
...
-- 4
DELIMITER //
CREATE PROCEDURE masukkan_data()
BEGIN
  SELECT * FROM transaksi WHERE MONTH(tanggal_transaksi) = 4 AND DAY(tanggal_transaksi) BETWEEN 3 AND 9 AND jumlah_barang > 1;
END //
DELIMITER ;
CALL masukkan_data;
...
CREATE PROCEDURE get_transaction_details()
  BEGIN
  SELECT t.*, p.nama_pelanggan FROM transaksi t INNER JOIN pembeli p ON t.id_pembeli = p.id_pembeli;
END;
DELIMITER;
