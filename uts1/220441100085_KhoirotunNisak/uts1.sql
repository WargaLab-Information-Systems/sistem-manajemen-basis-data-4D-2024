...
-1-
CREATE VIEW pesanan_diatasrata AS
SELECT
  nama.nama_pembeli,
  tra.total_harga,
  tra.tanggal_transaksi
FROM transaksi AS tra
JOIN pembeli AS nama ON tra.pembeli_id = nama.pembeli_id
WHERE tra.total_harga > (
  SELECT AVG(total_harga)
  FROM transaksi
);
SELECT*FROM pesanan_diatasrata

-2-
CREATE VIEW stok
AS SELECT nama_barang, stok_barang
FROM barang
...
SELECT
  tr.transaksi_id, pem.nama_pembeli, su.nama_supplier, ba.nama_barang,
  tr.jumlah_barang,tr.total_harga, tr.tanggal_transaksi
FROM transaksi tr
JOIN pembeli pem ON tr.pembeli_id = pem.pembeli_id
JOIN supplier su ON tr.supplier_id = su.supplier_id
JOIN barang AS ba ON tr.barang_id = ba.barang_id;
...
CREATE PROCEDURE barangdanstok()
BEGIN
  SELECT
    *
  FROM barang
...
OUT stokbarang INT (11))
BEGIN
  SELECT * FROM barang
  WHERE (SUM*barang.stok_barang)
END //
...
CREATE PROCEDURE masukkandata(
 ...
 alamat_sup VARCHAR (255)
 telepon_sup VARCHAR (20)
 )
BEGIN
  SELECT
    *FROM supplier
  INSERT INTO (supplier_id, nama_supplier, alamat_supplier, telepon_supplier);
END //
DELIMITER ;

CALL (1,'ninis','sampang','082')

-5-
DELIMITER //

CREATE PROCEDURE pembeli(
 id_sup INT(11)
 nama_sup VARCHAR (100)
 alamat_sup VARCHAR (255)
 telepon_sup VARCHAR (20)
...
CREATE PROCEDURE hapus(pake dropp tapi gatau wkwk
 id_sup INT(11)
 nama_sup VARCHAR (100)
 alamat_sup VARCHAR (255)
 telepon_sup VARCHAR (20)
