...
;
select * from ratarata;

-- 2.
create view stokkurang10 as
...
BEGIN
select * from barang 
where nama_barang like concat("%", huruf,"%") and stok_barang=stok;
END //
DELIMITER ;
...
in tanggal varchar(12),
in stok int(10)
)
BEGIN
select tanggal_transaksi from barang;
END //
DELIMITER ;
...
call total(@stokbarang);
select @stokbarang;

-- 4.
DELIMITER //
create procedure insertsupplier(
in idsupplier varchar(11),
in namasupplier varchar(100),
in alamatsupplier varchar(255),
in teleponesupplier varchar(25)
)
BEGIN
	insert into supplier(id_supplier, nama_supplier, alamat_supplier,telepone_supplier)
    values(idsupplier, namasupplier, alamatsupplier, teleponesupplier);
END //
DELIMITER ;

call supplier('20', 'nadia', 'nganjuk','085778888');
select supplier;
...
in idpembeli int(11),
in namapembeli varchar(100),
in alamatpembeli varchar(255),
in teleponpembeli varchar(20))
BEGIN
	update pembeli set
	where pembeli_id=idpembeli, nama_pembeli=namapembeli, alamat_pembeli=alamatpembeli, telepone_pembeli=teleponpembeli
END //
DELIMITER ;
call pembeli('20', 'adel', 'gresikk','085789999');
select pembeli;
