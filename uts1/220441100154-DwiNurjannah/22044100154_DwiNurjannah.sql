...
from barang join gudang on barang.barang_id=gudang.barang.id
where barang (stok_barang < 10 ));
...
in namabarang varchar(15),
in stokbarang int(10)
begin
		select*from  barang where nama_barang=namabarang  like concat("6" ,nama_barang,)
        and stok_databarang;
end //
call getdatabarang('e');
delimiter
call get data_barang('e');


delimiter//
create procedure input_supplier(
in nama_supplier varchar(50),
in alamat_supllier varchar(100),
in telepon_supplier varchar(20))
begin insert into input_supplier values (supplier_id,nama_supplier,alamat_supplier,telepon_supplier)
