...
FROM
    pembeli
JOIN
    transaksi ON pembeli.pembeli_id = transaksi.pembeli_id
WHERE
    transaksi.total_harga > (SELECT AVG(total_harga) FROM transaksi);
...
select 
    ...
    barang.stok_barang
from transaksi join barang  on transaksi.penjual_id = barang.penjual_id 
where transaksi.jumlah_barang ;
...
-- soal 4
delimiter //
create procedure tblsupplier(
IN supplier_id int (10),
IN nama_supplier varchar(100),
IN alamat_supplier varchar(100),
IN telepon_supplier varchar(100))
begin
	insert  into supplier values (supplier_id,nama_supplier,alamat_supplier,telepon_supplier) ;
    end //
    delimiter ;
    call tblSupplier ( 11,"sabil","jl Dahlia", "0852334456");
    
-- soal 3
DELIMITER //
CREATE PROCEDURE jumlahBarang(
    OUT jumlahBarang INT
)
BEGIN
    SELECT COUNT(*) INTO jumlahBarang
    FROM barang
    WHERE stok_barang;
END //
DELIMITER ;
call jumlahBarang(@jumlahBarang);
select @jumlahBarang;

-- soal 6
drop procedure tblsupplier;
