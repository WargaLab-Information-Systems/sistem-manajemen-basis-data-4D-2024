...
-- 1
DELIMITER //
create procedure databarang (namabarang varchar(20),
stok int(20))
begin 
select * from barang
where namabarang like concat('%', nama_barang, '%')
and stok_barang=stok;
END //
DELIMITER ;
CALL databarang ('e', 3);
...
-- 3
DELIMITER //
create procedure jumlahtotal (out total int(10))
BEGIN
SELECT * from barang
where stok_barang = total ;
END //
DELIMITER ;
CALL jumlahtotal (@total);

-- 4
DELIMITER //
create procedure inputsupplier (
in id varchar(5),
in nama varchar(20),
in alamat varchar(100),
in tpln int (20))
begin
insert into barang values (id, nama, alamat, tpln);
end //
DELIMITER ;
CALL inputsupplier ('013', 'hasnah','darjo', 087643683);
...
-- 6
DELIMITER //
CREATE PROCEDURE pembeli (nama varchar(20))
begin
drop  table pembeli;
end //
DELIMITER ;
