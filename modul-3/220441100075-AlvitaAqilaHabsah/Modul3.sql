...
CREATE TABLE petugas ( idPetugas VARCHAR (10) PRIMARY KEY, Username VARCHAR (15), PASSWORD VARCHAR(15), Nama VARCHAR (25));
CREATE TABLE buku ( kode_buku VARCHAR (10) PRIMARY KEY, judul_buku VARCHAR (25), pengarang_buku VARCHAR(30), penerbit_buku VARCHAR (30),
	tahun_buku VARCHAR (10), jumlah_buku VARCHAR (5), status_buku VARCHAR (10), klasifikasi_buku VARCHAR (20));
CREATE TABLE anggota ( idAnggota VARCHAR (10) PRIMARY KEY, nama_anggota VARCHAR (20), angkatan_anggota VARCHAR(6), tempat_lahir_anggota VARCHAR (20),
	tanggal_lahir_anggota DATE, no_telp VARCHAR (12), jenis_kelamin VARCHAR (15), status_pinjam VARCHAR (15));
CREATE TABLE peminjaman( kode_peminjaman VARCHAR (10) PRIMARY KEY, idAnggota VARCHAR (10), idPetugas VARCHAR (10), tanggal_pinjam DATE, 
	tanggal_kembali DATE, kode_buku VARCHAR (10), FOREIGN KEY (idAnggota) REFERENCES anggota(idAnggota),
	FOREIGN KEY (idPetugas) REFERENCES petugas(idPetugas), FOREIGN KEY (kode_buku) REFERENCES buku(kode_buku));
CREATE TABLE pengembalian( kode_kembali VARCHAR (10) PRIMARY KEY, idAnggota VARCHAR (10), kode_buku VARCHAR (10), idPetugas VARCHAR (10), 
	tgl_pinjam DATE, tgl_kembali DATE, denda VARCHAR (15), FOREIGN KEY (idAnggota) REFERENCES anggota(idAnggota),
	FOREIGN KEY (idPetugas) REFERENCES petugas(idPetugas), FOREIGN KEY (kode_buku) REFERENCES buku(kode_buku));
...
END //
DELIMITER ;
CALL tambah_petugas ('PTG011', 'Sarita', 'Sari11', 'Sarita Ayu');
SELECT * FROM petugas

-- data pada table berdasarkan pada salah satu field yaitu kode buku
DELIMITER //
CREATE PROCEDURE GetDatabuku(IN buku_kode VARCHAR(10))
BEGIN
    SELECT kode_buku FROM buku WHERE kode_buku LIKE CONCAT('%' , buku_kode, '%'); 
END //
DELIMITER ;
CALL GetDatabuku ('BK');

SELECT kode_buku FROM buku WHERE kode_buku LIKE '%BK%'
DROP PROCEDURE GetDatabuku;

-- data pada table anggota berdasarkan dua field yaitu angkatan dan jenis kelamin.
DELIMITER //
CREATE PROCEDURE TampilkanAnggota(IN angkatan VARCHAR(6), IN jenis_kelamin_anggota VARCHAR(15))
BEGIN
...
-- data pada table buku berdasarkan pada 3 field yaitu pengarang, penerbit dan tahun terbit.
DELIMITER //
...
-- memasukkan data pada table buku.
DELIMITER //
CREATE PROCEDURE input_buku(IN kode VARCHAR(10), IN judul VARCHAR(25), IN pengarang VARCHAR(30), IN penerbit VARCHAR(30),
    IN tahun VARCHAR(10), IN jumlah VARCHAR(5), IN STATUS VARCHAR(10), IN klasifikasi VARCHAR(20))
BEGIN
...
-- mengetahui data jumlah buku menggunakan parameter OUT.
DELIMITER //
...
-- mengetahui data jumlah anggota berdasarkan angkatan dan jenis kelaminnya menggunakan parameter INOUT.
DELIMITER //
