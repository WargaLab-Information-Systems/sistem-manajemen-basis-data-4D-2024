...
SELECT 
    ...
    pes.total, 
    pes.tanggal_pesanan,
    (SELECT AVG(total) FROM tb_pesanan) AS rerata_total_pesanan
FROM tb_pesanan pes 
...
SELECT p.nama_pelanggan, pes.tanggal_pesanan, SUM(pes.total) AS total_pesanan
FROM tb_pesanan pes
JOIN tb_pelanggan p ON pes.id_pelanggan = p.id_pelanggan
...
SELECT p.id_pelanggan, pr.id_produk, pr.nama_produk, dp.jumlah
FROM tb_pelanggan p
